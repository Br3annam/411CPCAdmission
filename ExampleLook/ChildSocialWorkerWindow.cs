...
namespace ExampleLook
{
    public partial class ChildSocialWorkerWindow : Form
    {
        ...
        private void getChildSocial()
        {
            ...
            cSWEduRiskWeapComboBox.SelectedIndex = int.Parse(dl.cSWEduRiskWeap);
            cSWWeapHomeComboBox.SelectedIndex = int.Parse(dl.cSWWeapHome);
            cSWPastSelfInjComboBox.SelectedIndex = int.Parse(dl.cSWPastSelfInj);
            cSWSrcReliabComboBox.SelectedIndex = int.Parse(dl.cSWSrcReliab);
            cSWLegStatComboBox.SelectedIndex = int.Parse(dl.cSWLegStat);
            cSWHistCYSComboBox.SelectedIndex = int.Parse(dl.cSWHistCYS);
            cSWCarFeelPatDifBehavComboBox.SelectedIndex = int.Parse(dl.cSWCarFeelPatDifBehav);
            cSWMemHelpPatComboBox.SelectedIndex = int.Parse(dl.cSWMemHelpPat);
            cSWParentMaritStatComboBox.SelectedIndex = int.Parse(dl.cSWParentMaritStat);
            cSWSexActPregComboBox.SelectedIndex = int.Parse(dl.cSWSexActPreg);
            cSWSexActComboBox.SelectedIndex = int.Parse(dl.cSWSexAct);
            cSWSafeHomeComboBox.SelectedIndex = int.Parse(dl.cSWSafeHome);
            cSWRetHomeComboBox.SelectedIndex = int.Parse(dl.cSWRetHome);
            cSWEmoNegAbuseReportComboBox.SelectedIndex = int.Parse(dl.cSWEmoNegAbuseReport);
            cSWEmoNegAbuseWitnessComboBox.SelectedIndex = int.Parse(dl.cSWEmoNegAbuseWitness);
            cSWEmoNegAbusePerpComboBox.SelectedIndex = int.Parse(dl.cSWEmoNegAbusePerp);
            cSWEmoNegAbuseVictComboBox.SelectedIndex = int.Parse(dl.cSWEmoNegAbuseVict);
            cSWPsyAbuseReportComboBox.SelectedIndex = int.Parse(dl.cSWPsyAbuseReport);
            cSWPsyAbuseWitnessComboBox.SelectedIndex = int.Parse(dl.cSWPsyAbuseWitness);
            cSWPsyAbusePerpComboBox.SelectedIndex = int.Parse(dl.cSWPsyAbusePerp);
            cSWPsyAbuseVictComboBox.SelectedIndex = int.Parse(dl.cSWPsyAbuseVict);
            cSWSexAbuseReportComboBox.SelectedIndex = int.Parse(dl.cSWSexAbuseReport);
            cSWSexAbuseWitnessComboBox.SelectedIndex = int.Parse(dl.cSWSexAbuseWitness);
            cSWSexAbusePerpComboBox.SelectedIndex = int.Parse(dl.cSWSexAbusePerp);
            cSWSexAbuseVictComboBox.SelectedIndex = int.Parse(dl.cSWSexAbuseVict);
            cSWOthLegIssComboBox.SelectedIndex = int.Parse(dl.cSWOthLegIss);
            cSWHistArrestComboBox.SelectedIndex = int.Parse(dl.cSWHistArrest);
            cSWSeeUseProbComboBox.SelectedIndex = int.Parse(dl.cSWSeeUseProb);
            cSWIncUseSubstanceComboBox.SelectedIndex = int.Parse(dl.cSWIncUseSubstance);
            cSWPatHospPrevComboBox.SelectedIndex = int.Parse(dl.cSWPatHospPrev);
            cSWPatMedTreatProvComboBox.SelectedIndex = int.Parse(dl.cSWPatMedTreatProv);
            cSWPatMedPatComboBox.SelectedIndex = int.Parse(dl.cSWPatMedPat);
            cSWHistHomSuicComboBox.SelectedIndex = int.Parse(dl.cSWHistHomSuic);
            cSWFamMemAbuImpComboBox.SelectedIndex = int.Parse(dl.cSWFamMemAbuImp);
            cSWCultOtherComboBox.SelectedIndex = int.Parse(dl.cSWCultOther);
            cSWCultIssContribProbComboBox.SelectedIndex = int.Parse(dl.cSWCultIssContribProb);
            cSWChangeReligInvolvComboBox.SelectedIndex = int.Parse(dl.cSWChangeReligInvolv);
            cSWEatDepBoredComboBox.SelectedIndex = int.Parse(dl.cSWEatDepBored);
            cSWHistSigMedIssComboBox.SelectedIndex = int.Parse(dl.cSWHistSigMedIss);
            cSWChronPainDisabImpFunctComboBox.SelectedIndex = int.Parse(dl.cSWChronPainDisabImpFunct);
            cSWMedIssContribComboBox.SelectedIndex = int.Parse(dl.cSWMedIssContrib);
            cSWBasicNeedMetComboBox.SelectedIndex = int.Parse(dl.cSWBasicNeedMet);
            cSWParMilitComboBox.SelectedIndex = int.Parse(dl.cSWParMilit);
            cSWWorkingComboBox.SelectedIndex = int.Parse(dl.cSWWorking);
            cSWDetSuspensTruancyComboBox.SelectedIndex = int.Parse(dl.cSWDetSuspensTruancy);
            cSWSigChanSocAcaComboBox.SelectedIndex = int.Parse(dl.cSWSigChanSocAca);
            cSWConcernFollDischComboBox.SelectedIndex = int.Parse(dl.cSWConcernFollDisch);
            cSWMACommSuppServComboBox.SelectedIndex = int.Parse(dl.cSWMACommSuppServ);
        }
        ...
        private void setChildSocial()
        {
            ...
            dl.cSWEduRiskWeap = cSWEduRiskWeapComboBox.SelectedIndex.ToString();
            dl.cSWWeapHome = cSWWeapHomeComboBox.SelectedIndex.ToString();
            dl.cSWPastSelfInj = cSWPastSelfInjComboBox.SelectedIndex.ToString();
            dl.cSWSrcReliab = cSWSrcReliabComboBox.SelectedIndex.ToString();
            dl.cSWLegStat = cSWLegStatComboBox.SelectedIndex.ToString();
            dl.cSWHistCYS = cSWHistCYSComboBox.SelectedIndex.ToString();
            dl.cSWCarFeelPatDifBehav = cSWCarFeelPatDifBehavComboBox.SelectedIndex.ToString();
            dl.cSWMemHelpPat = cSWMemHelpPatComboBox.SelectedIndex.ToString();
            dl.cSWParentMaritStat = cSWParentMaritStatComboBox.SelectedIndex.ToString();
            dl.cSWSexActPreg = cSWSexActPregComboBox.SelectedIndex.ToString();
            dl.cSWSexAct = cSWSexActComboBox.SelectedIndex.ToString();
            dl.cSWSafeHome = cSWSafeHomeComboBox.SelectedIndex.ToString();
            dl.cSWRetHome = cSWRetHomeComboBox.SelectedIndex.ToString();
            dl.cSWEmoNegAbuseReport = cSWEmoNegAbuseReportComboBox.SelectedIndex.ToString();
            dl.cSWEmoNegAbuseWitness = cSWEmoNegAbuseWitnessComboBox.SelectedIndex.ToString();
            dl.cSWEmoNegAbusePerp = cSWEmoNegAbusePerpComboBox.SelectedIndex.ToString();
            dl.cSWEmoNegAbuseVict = cSWEmoNegAbuseVictComboBox.SelectedIndex.ToString();
            dl.cSWPsyAbuseReport = cSWPsyAbuseReportComboBox.SelectedIndex.ToString();
            dl.cSWPsyAbuseWitness = cSWPsyAbuseWitnessComboBox.SelectedIndex.ToString();
            dl.cSWPsyAbusePerp = cSWPsyAbusePerpComboBox.SelectedIndex.ToString();
            dl.cSWPsyAbuseVict = cSWPsyAbuseVictComboBox.SelectedIndex.ToString();
            dl.cSWSexAbuseReport = cSWSexAbuseReportComboBox.SelectedIndex.ToString();
            dl.cSWSexAbuseWitness = cSWSexAbuseWitnessComboBox.SelectedIndex.ToString();
            dl.cSWSexAbusePerp = cSWSexAbusePerpComboBox.SelectedIndex.ToString();
            dl.cSWSexAbuseVict = cSWSexAbuseVictComboBox.SelectedIndex.ToString();
            dl.cSWOthLegIss = cSWOthLegIssComboBox.SelectedIndex.ToString();
            dl.cSWHistArrest = cSWHistArrestComboBox.SelectedIndex.ToString();
            dl.cSWSeeUseProb = cSWSeeUseProbComboBox.SelectedIndex.ToString();
            dl.cSWIncUseSubstance = cSWIncUseSubstanceComboBox.SelectedIndex.ToString();
            dl.cSWPatHospPrev = cSWPatHospPrevComboBox.SelectedIndex.ToString();
            dl.cSWPatMedTreatProv = cSWPatMedTreatProvComboBox.SelectedIndex.ToString();
            dl.cSWPatMedPat = cSWPatMedPatComboBox.SelectedIndex.ToString();
            dl.cSWHistHomSuic = cSWHistHomSuicComboBox.SelectedIndex.ToString();
            dl.cSWFamMemAbuImp = cSWFamMemAbuImpComboBox.SelectedIndex.ToString();
            dl.cSWCultOther = cSWCultOtherComboBox.SelectedIndex.ToString();
            dl.cSWCultIssContribProb = cSWCultIssContribProbComboBox.SelectedIndex.ToString();
            dl.cSWChangeReligInvolv = cSWChangeReligInvolvComboBox.SelectedIndex.ToString();
            dl.cSWEatDepBored = cSWEatDepBoredComboBox.SelectedIndex.ToString();
            dl.cSWHistSigMedIss = cSWHistSigMedIssComboBox.SelectedIndex.ToString();
            dl.cSWChronPainDisabImpFunct = cSWChronPainDisabImpFunctComboBox.SelectedIndex.ToString();
            dl.cSWMedIssContrib = cSWMedIssContribComboBox.SelectedIndex.ToString();
            dl.cSWBasicNeedMet = cSWBasicNeedMetComboBox.SelectedIndex.ToString();
            dl.cSWParMilit = cSWParMilitComboBox.SelectedIndex.ToString();
            dl.cSWWorking = cSWWorkingComboBox.SelectedIndex.ToString();
            dl.cSWDetSuspensTruancy = cSWDetSuspensTruancyComboBox.SelectedIndex.ToString();
            dl.cSWSigChanSocAca = cSWSigChanSocAcaComboBox.SelectedIndex.ToString();
            dl.cSWConcernFollDisch = cSWConcernFollDischComboBox.SelectedIndex.ToString();
            dl.cSWMACommSuppServ = cSWMACommSuppServComboBox.SelectedIndex.ToString();
        }
